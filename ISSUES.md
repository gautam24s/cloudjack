...
- [x] **1. `GCPConfig.project_id` can be `None` but services use it as `str`**
  ...
  - **Files:** `cloudjack/gcp/compute.py`, `cloudjack/gcp/dns.py`, `cloudjack/gcp/iam.py`, `cloudjack/gcp/logging_service.py`, `cloudjack/gcp/queue.py`, `cloudjack/gcp/secret_manager.py`, `cloudjack/gcp/storage.py`
...
- [x] **2. `GCPConfig.credentials_path` is resolved but never used**
  `cloudjack/base/config.py` resolves `GOOGLE_APPLICATION_CREDENTIALS` into `credentials_path`, but no GCP service reads `config.credentials_path` to load credentials from the file. The field is dead config — users who set `GOOGLE_APPLICATION_CREDENTIALS` might expect it to work through cloudjack, but it only works if the GCP SDK picks it up independently.
...
- [x] **3. `ClientCache` is not thread-safe despite docstring claim**
  `cloudjack/base/client_cache.py` says "Thread-safe, in-process cache" but uses a plain `dict` with no locking. Concurrent `get_or_create` calls can create duplicate instances or see partial state.
...
- [x] **5. GCP services don't pass `credentials` to SDK clients**
  ...
  - `cloudjack/gcp/compute.py` — `compute_v1.InstancesClient()` — no credentials passed
  - `cloudjack/gcp/dns.py` — `cloud_dns.Client(project=...)` — no credentials passed
  - `cloudjack/gcp/iam.py` — `iam_admin_v1.IAMClient()` — no credentials passed
  - `cloudjack/gcp/logging_service.py` — `cloud_logging.Client(project=...)` — no credentials passed
  - `cloudjack/gcp/queue.py` — `PublisherClient()` / `SubscriberClient()` — no credentials passed

  Only `cloudjack/gcp/secret_manager.py` and `cloudjack/gcp/storage.py` pass `credentials`.
...
- [x] **6. AWS `list_zones` / `list_log_groups` / `list_roles` / `list_policies` don't paginate**
  - `cloudjack/aws/dns.py` — `list_hosted_zones()` returns only the first page (max 100 zones).
  - `cloudjack/aws/logging_service.py` — `describe_log_groups()` returns only first page.
  - `cloudjack/aws/iam.py` — `list_roles()` returns only first page.
  - `cloudjack/aws/iam.py` — `list_policies()` returns only first page.

  In contrast, `list_objects` in `cloudjack/aws/storage.py` correctly uses a paginator.
...
- [x] **7. AWS `read_logs` severity is hardcoded**
  `cloudjack/aws/logging_service.py` — severity is always `"INFO"` regardless of actual log content. The `write_log` method prepends `[{severity}]` to the message, but `read_logs` doesn't parse it back out.
  - **Fix:** `read_logs` now parses the `[SEVERITY] message` format to extract severity and clean message.

- [x] **8. `cloudjack/__init__.py` only exports `SecretManagerService`**
...
- [x] **9. `cloudjack/aws/__init__.py` and `cloudjack/gcp/__init__.py` only export 2 of 7 services**
...
- [x] **11. `InstanceAlreadyExistsError` imported inline in GCP compute**
  `cloudjack/gcp/compute.py` — imported inside the `except` block rather than at the top with other imports.
  - **Fix:** Moved import to top-level with other exception imports.

- [x] **12. `retry` decorator has unreachable final `raise`**
  `cloudjack/base/retry.py` — `raise last_exc` after the for loop is technically unreachable. The `# type: ignore[misc]` comment confirms this.
...
- [x] **13. `CloudjackLogger` handler duplication risk**
  `cloudjack/base/logger.py` — checks `if not self.logger.handlers` but `CloudjackLogger` is not a singleton. Multiple instances with the same name share the stdlib logger, making the check work but the design fragile.
...
- [x] **14. `generate_signed_url` in base blueprint uses untyped `**kwargs`**
  `cloudjack/base/storage.py` — `**kwargs` parameter is not typed (`**kwargs` instead of `**kwargs: Any`), inconsistent with all other blueprints.
...
- [x] **15. `SecretManagerService` methods have redundant `pass` bodies**
  `cloudjack/base/secret_manager.py` — all abstract methods have explicit `pass` statements after docstrings, unlike the other blueprints.
  - **Fix:** Removed all redundant `pass` statements.

- [x] **16. No `__all__` in `cloudjack/base/config.py`**
